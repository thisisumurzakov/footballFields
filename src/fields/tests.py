...
class FieldsAppTests(APITestCase):
    ...
    def test_user_can_create_booking(self):
        ...
        start_time = (timezone.now() + timedelta(hours=2)).replace(
    ...
    def test_booking_with_past_start_time(self):
        self.client.force_authenticate(user=self.user)
        url = reverse("booking-list")
        start_time = (timezone.now() - timedelta(hours=1)).replace(
            microsecond=0, second=0, minute=0
        )
        end_time = (timezone.now() + timedelta(hours=1)).replace(
            microsecond=0, second=0, minute=0
        )
        data = {
            "field": self.field.id,
        ...
        self.assertIn(
            "Start time must be in the future.", response.data["non_field_errors"][0]
        )

    def test_booking_duration_not_multiple_of_min_duration(self):
        self.client.force_authenticate(user=self.user)
        url = reverse("booking-list")
        start_time = (timezone.now() + timedelta(hours=3)).replace(
            microsecond=0, second=0, minute=0
        )
        ...
        self.assertIn("non_field_errors", response.data)
        self.assertIn(
            "Booking duration must be a multiple of the minimum booking duration",
    ...
    def test_booking_overlapping(self):
        ...
        existing_start_time = (timezone.now() + timedelta(hours=4)).replace(
            microsecond=0, second=0, minute=0
        )
        existing_end_time = existing_start_time + timedelta(
            hours=2
        )  # Booking from 4 PM to 6 PM
        ...
        self.assertIn("non_field_errors", response.data)
        self.assertIn(
            "This field is already booked for the given time.",
            response.data["non_field_errors"][0],
        )

    def test_user_can_retrieve_own_bookings(self):
        booking_start_time = (timezone.now() + timedelta(hours=3)).replace(
            microsecond=0, second=0, minute=0
        )
    ...
    def test_user_cannot_delete_others_booking(self):
        ...
        booking_start_time = (timezone.now() + timedelta(hours=4)).replace(
            microsecond=0, second=0, minute=0
        )
        booking_end_time = booking_start_time + timedelta(hours=2)
    ...
    def test_get_available_fields(self):
        ...
        booking_end_time = booking_start_time + timedelta(
            hours=2
        )  # Booking from 4 PM to 6 PM
        Booking.objects.create(
            field=self.field,
    ...
    def test_get_available_fields_sorted_by_distance(self):
        ...
        self.assertEqual(
            response.data[1]["name"], "Test Field 2"
        )  # Farther in Los Angeles
